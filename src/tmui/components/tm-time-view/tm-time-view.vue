<template>
	<view class="flex relative flex-col" :style="{height:props.height+'rpx'}">
		<picker-view v-if="show"  :value="colIndex" @change="colchange" 
		...
		>
				<picker-view-column v-if="showCol.year">
					...
					:class="[colIndex[0]==index?'itemSelected':'UnitemSelected']" >
						<TmText  :font-size="30" :dark="isDark" :label="item+showSuffix['year']"></TmText>
					</view>
				</picker-view-column>
				<picker-view-column v-if="showCol.month">
					<view  v-for="(item,index) in _col.month" 
					:key="index" class="flex itemcel flex-row flex-row-center-center" 
					:class="[colIndex[1]==index?'itemSelected':'UnitemSelected']" >
				...
				<picker-view-column v-if="showCol.date">
					...
					:class="[colIndex[2]==index?'itemSelected':'UnitemSelected']" >
						<TmText  :font-size="30" :dark="isDark" :label="item+showSuffix['date']"></TmText>
					</view>
				</picker-view-column>
				<picker-view-column v-if="showCol.hour">
					<view  v-for="(item,index) in _col.hour" 
					:key="index" class="flex itemcel flex-row flex-row-center-center" 
					:class="[colIndex[3]==index?'itemSelected':'UnitemSelected']" >
						<TmText  :font-size="30" :dark="isDark" :label="item+showSuffix['hour']"></TmText>
					</view>
				</picker-view-column>
				<picker-view-column v-if="showCol.minute">
					<view  v-for="(item,index) in _col.minute" 
					:key="index" class="flex itemcel flex-row flex-row-center-center" 
					:class="[colIndex[4]==index?'itemSelected':'UnitemSelected']" >
						<TmText  :font-size="30" :dark="isDark" :label="item+showSuffix['minute']"></TmText>
					</view>
				</picker-view-column>
				<picker-view-column v-if="showCol.second">
					<view  v-for="(item,index) in _col.second" 
					:key="index" class="flex itemcel flex-row flex-row-center-center" 
					:class="[colIndex[5]==index?'itemSelected':'UnitemSelected']" >
						<TmText  :font-size="30" :dark="isDark" :label="item+showSuffix['second']"></TmText>
					</view>
				</picker-view-column>
...
import { computed, PropType, getCurrentInstance,ref, toRaw,onMounted,nextTick, watch,onUpdated, Ref } from 'vue';
...
import {rangeTimeArray,getNowbyIndex,getIndexNowbydate,checkNowDateisBetween} from "./time"
