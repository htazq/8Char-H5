<template>
	<view class="flex flex-row relative">
		<view v-if="show" @click.stop="closeDromenu" class="l-0 t-0 fixed zIndex-9" 
		:style="[{width:windowWidth+'px',height:windowHeight+'px',background:'rgba(0,0,0,0)'}]">
		</view>
		<view class="flex flex-col "
			...
			<view @click.stop="openDromenu"  class="relative zIndex-1 flex flex-row">
				<view :eventPenetrationEnabled="false" :userInteractionEnabled="false" ref="popver" 
				class=" flex flex-row"><slot></slot></view>
			</view>
			<view v-if="show" style="z-index:400"
				:class="[
