...
export const text = {
	...
	draw({
		...
		// #ifdef APP-NVUE
		ctx.setFontSize(fontSize*ctx.dpr)
		// #endif
		...
		position = new Array(rowNum).fill(0).map((foo, i) => [Math.ceil(x), Math.ceil(y + i * lineHeight - offset)])
		
		if (typeof maxWidth == 'undefined' || !maxWidth || isNaN(maxWidth)){
			 maxWidth = w
		}
		// #ifdef H5
...
export function newGraph(name, config) {
	if (!name || !config) {
		console.error('ExtendNewGraph Missing Parameters!')

		return
	}
	config = {...graphs.get(name),...config}
